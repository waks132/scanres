...
- IDS Dynamique avec Online Learning (ADWIN)
- Transfer Learning avancé (Procrustes)
- Support GPU (CuPy - optionnel)
- Numba JIT pour fonctions critiques (optionnel)

Modules:
    network_svd_analyzer.py      - Moteur SVD/PCA optimisé (v2.0)
    optimized_offensive_engine.py - Intelligence offensive proactive (v2.0)
...
def run_basic_mode(config, profiler):
    ...
    perturbations, impacts = analyzer.model_stochastic_perturbations(n_perturbations=config.N_PERTURBATIONS)
...
def run_offensive_mode(config, profiler):
    ...
    surfaces = intelligence.proactive_vulnerability_scan(use_nonlinear=True)
    ...
    evasion = simulator.simulate_adaptive_evasion(surfaces[0], n_episodes=config.N_EVASION_EPISODES, use_rl=True)
